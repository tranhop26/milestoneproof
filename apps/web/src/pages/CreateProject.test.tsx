...
function connectedProvider(): Eip1193Provider {
  return {
    request: vi.fn(async ({ method }) => {
      if (method === "eth_accounts" || method === "eth_requestAccounts") return [SPONSOR]
...
function fakeContract(readbackGate = deferred<void>()): MilestoneProofContract {
  return {
    address: CONTRACT,
    reads: {
      config: vi.fn(),
...
async function completeRequiredFields(builder: string = BUILDER) {
  ...
  await user.type(screen.getByLabelText("Milestone 1 deadline"), "2030-03-17T12:00")
  return user
}
